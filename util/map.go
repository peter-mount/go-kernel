...
type Map[V any] interface {
	...
	Get(k string) V
	// Get2 returns a value from the map or nil if absent
	Get2(k string) (V, bool)
	...
	RemoveIfEquals(k string, v V) bool
	...
	ComputeIfAbsent(k string, f func(string) V) V
	...
	Merge(k string, v V, f func(V, V) V) V
	...
	// outside any lock as long as the value exists at the time the function would be called.
	...
	ExecIfPresent(k string, f func(V))
}
